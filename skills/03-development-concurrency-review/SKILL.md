...
metadata:
  version: "1.1.0"
  domain: backend
  triggers: concurrency, thread safety, race conditions, deadlocks, Virtual Threads, CompletableFuture, async, multithreading
  role: reviewer
  scope: review
---
...
- `java-development` — Java 25 Virtual Threads, structured concurrency, and `ScopedValue` replace many classic concurrency patterns
- `spring-boot` — Virtual Threads configuration in Spring Boot 4 (`spring.threads.virtual.enabled=true`)
- `performance-optimization` — Concurrency issues are a primary category of performance bottlenecks
- `debugging-and-error-recovery` — Concurrency bugs (races, deadlocks) require specialized diagnostic techniques
- `observability` — Thread pool metrics (HikariCP active connections, executor queue depth) expose concurrency health
