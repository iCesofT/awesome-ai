...
metadata:
  version: "1.1.0"
  domain: architecture
  triggers: DDD, Clean Architecture, Hexagonal Architecture, ports and adapters, domain events, CQRS, aggregates, value objects, bounded context
  role: architect
  scope: design
---
...
- `ddd-strategic-design` — Strategic patterns (bounded contexts, context maps) inform the module structure
- `spring-boot` — Multi-module Maven setup maps directly to the hexagonal module layout
- `openapi-spec-generation` — REST adapter generated from OpenAPI spec using delegate pattern
- `grpc-design` — gRPC adapter sharing the same application layer as the REST adapter
- `jpa-patterns` — JPA repositories implement the persistence port in the infrastructure layer
- `caching-patterns` — Cache adapters sit in the infrastructure layer, accessed via application-layer ports
