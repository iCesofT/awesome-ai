...
metadata:
  version: "1.1.0"
  domain: tools
  triggers: changelog, release notes, git log, conventional commits, release, version history
  role: advisor
...
- `conventional-commit` — Conventional commits are the structured input format that makes changelog generation possible
- `git-commit` — Clean commit discipline (atomic commits, meaningful messages) directly improves changelog quality
- `deployment-patterns` — Changelogs are published as part of the release pipeline alongside deployment artifacts
- `architecture-adrs` — Notable architectural changes in the changelog should reference the corresponding ADR
