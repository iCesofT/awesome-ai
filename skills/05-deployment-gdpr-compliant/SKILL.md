...
- `springboot-security` — GDPR compliance relies on security primitives: encryption at rest/transit, access control, audit logging
- `observability` — Logs and traces must not contain PII; structured logging with MDC filters is the enforcement mechanism
- `liquibase` — Data retention and anonymization policies are implemented via Liquibase migration scripts
- `security-audit` — GDPR audit is a specialized subset of the broader security audit covering personal data handling
- `deployment-patterns` — Data residency and environment isolation requirements affect deployment architecture
