...
metadata:
  version: "1.1.0"
  domain: database
  triggers: SQL review, SQL security, SQL injection, SQL code quality, SQL standards, database review
  role: reviewer
  scope: review
---
...
- `sql-optimization` — Code review identifies performance smells; optimization provides the fix techniques
- `jpa-patterns` — JPA generates SQL; review both the JPQL and the emitted SQL for N+1, missing joins
- `liquibase` — Schema migrations are SQL artifacts subject to the same code review standards
- `security-audit` — SQL injection is the top database security risk; always verify parameterization
- `performance-smell-detection` — SQL smells (SELECT *, missing WHERE, cartesian joins) are a subset of performance smells
