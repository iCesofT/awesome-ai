...
metadata:
  version: "1.1.0"
  domain: testing
  triggers: JUnit 5, JUnit 6, AssertJ, test quality, unit testing, test coverage, write tests
  role: specialist
...
- `test-driven-development` — TDD generates tests; test-quality principles keep them maintainable as the codebase grows
- `bdd-patterns` — BDD acceptance tests are evaluated by the same quality standards: readability, isolation, determinism
- `spring-boot-testing` — Integration test quality: avoid over-mocking, use Testcontainers, scope `@SpringBootTest` narrowly
- `code-review-and-quality` — Test code is reviewed with the same rigor as production code; quality standards apply equally
- `jpa-patterns` — Test quality for JPA: prefer `@DataJpaTest` over full context, assert SQL efficiency not just results
