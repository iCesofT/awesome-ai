...
metadata:
  version: "1.1.0"
  domain: tools
  triggers: git commit, commit message, conventional commits, version control
  role: advisor
...
- `conventional-commit` — Conventional commit format is the standard for structured, machine-readable commit messages
- `changelog-generator` — Git history with conventional commits drives automated changelog generation
- `code-review-and-quality` — Small, focused commits make code reviews faster and easier to revert
- `deployment-patterns` — Clean git history (squash, tagging) supports reliable release automation
