...
metadata:
  version: "1.1.0"
  domain: backend
  triggers: performance smells, Java performance, streams, collections, boxing, regex, object creation, code review
  role: reviewer
  scope: review
---
...
- `performance-optimization` — Smell detection finds the candidates; optimization provides the fix strategies
- `sql-optimization` — SQL smells (N+1, SELECT *, missing indexes) are a major category of detected smells
- `jpa-patterns` — JPA N+1 queries and lazy-loading issues are the most common Java performance smells
- `caching-patterns` — Repeated identical queries without caching are a detectable performance smell
- `concurrency-review` — Thread contention, synchronized blocks, and blocking I/O on Virtual Threads are concurrency smells
