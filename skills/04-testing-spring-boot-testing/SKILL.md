...
metadata:
  version: "1.1.0"
  domain: testing
  triggers: Spring Boot testing, JUnit 6, AssertJ, MockMvc, Testcontainers, test slice, integration testing
  role: specialist
...
- `bdd-patterns` — BDD scenarios with Cucumber 7 run as integration tests in the Spring Boot testing layer
- `jpa-patterns` — `@DataJpaTest` with Testcontainers is the standard for repository integration tests
- `liquibase` — Testcontainers runs real migrations before each test; verifies migration correctness
- `test-driven-development` — TDD drives implementation; Spring Boot testing validates integration behavior
- `test-quality` — Same quality standards apply to Spring Boot tests: focused, deterministic, well-named
