...
metadata:
  version: "1.1.0"
  domain: architecture
  triggers: ADR, architectural decision, design decision, architecture record, decision log
  role: architect
  scope: design
---
...
- `ddd-strategic-design` — ADRs often document bounded context decisions and integration pattern choices
- `clean-ddd-hexagonal` — Record architectural decisions about module structure, adapters, and ports
- `architecture-review` — Use ADRs as output of architecture review findings requiring documented decisions
- `deployment-patterns` — Infrastructure and deployment choices (blue/green, Kubernetes) deserve their own ADRs
- `springboot-security` — Security architecture decisions (auth strategy, token storage) must be documented as ADRs
